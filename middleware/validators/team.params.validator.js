...
module.exports.name = async (ctx, next) => {
  ctx.request.body.name = _checkText(ctx.request?.body?.name) || '';
  await next();
};

module.exports.position = async (ctx, next) => {
  ctx.request.body.position = _checkText(ctx.request?.body?.position) || '';
  await next();
};
...
module.exports.objectId = async (ctx, next) => {
  if (!isValidObjectId(ctx.params.id)) {
    _deleteFile(ctx.request.files || []);
    ctx.throw(400, 'invalid team unit id');
  }
